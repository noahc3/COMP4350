...
export const Sidebar = observer(() => {
  ...
  const allButtons = allSpools?.map(function (spool) {
    return (
      <NavLink to={"/s/" + spool.name} key={spool.id}>
        <Button
          textColor={"white"}
          colorScheme={mode("purple.500", "purple.900")(colorMode)}
        >
          {spool.name}
        </Button>
      </NavLink>
  ...
  return (
    <Flex
      direction={"column"}
      className="sidebar"
      bgColor={mode("purple.500", "purple.900")(colorMode)}
    >
      ...
      {isAuthenticated && (
        <>
          <Button
            leftIcon={<Icon transform={"scaleX(-1)"} as={MdOutlineExitToApp} />}
            textColor="white"
            colorScheme={mode("purple.500", "purple.900")(colorMode)}
            onClick={() => {
              logout();
