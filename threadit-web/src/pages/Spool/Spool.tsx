...
export const Spool = observer(() => {
  ...
  const handleInputChange = (newValue: string) => {
    setSelectOption([
      {
        value: newValue,
        label: `Search for "${newValue}" in s/${spool ? spool.name : ""}`,
      },
    ]);
  };
  ...
  const handleSearch = (
    ...
    if (spool && selectedOption) {
      navStore.navigateTo(
        `/s/${spool.name}${sortType ? "/" + sortType : ""}/?q=${
  ...
  React.useEffect(() => {
    if (id) {
      SpoolAPI.getSpoolByName(id).then((spool) => {
        setSelectOption([
          { value: "", label: `Search for "" in s/${spool ? spool.name : ""}` },
        ]);
        setSpool(spool);
  ...
  React.useEffect(() => {
    ...
    if (id && profile) {
      (async () => {
        setBelongs(await UserSettingsAPI.getJoinedSpool(id!));
      })();
    }
  }, [id, profile]);
          ...
          <VStack>
            {isAuthenticated && (
              <VStack w="100%">
                ...
                >
                  <HStack>
                    <NavLink to={"/s/" + spool.name + "/createthread"}>
                      <Button leftIcon={<IoCreateOutline />} colorScheme="blue">
                        Create Post
                      </Button>
                    </NavLink>
                        ...
                        {belongs ? (
                          ...
                          <>
                            <Button
                              ...
                              leftIcon={<CheckIcon />}
                              colorScheme={isLoadingBelongs ? "gray" : "green"}
                              onClick={() => {
                                joinSpool();
                              }}
                            >
                    ...
                    {spool.ownerId === profile?.id && (
                      <>
                        <Spacer />
                        <NavLink to={"/s/" + spool.name + "/manage"}>
                          <Button
